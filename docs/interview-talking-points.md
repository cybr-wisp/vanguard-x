...
The frozen benchmark does not claim one artificial saturation threshold. Under the operational 300 m spatial grid, measured throughput was 48,858 reports/s at 50 targets, 21,348 at 200, 14,962 at 500, and 16,696 at 1,000. I do not claim monotonic scaling from those measured points. At 200 targets, measured in-process tracking latency was p50 13.49 ms, p95 18.45 ms, and p99 20.73 ms. In the current implementation, sustained Kafka consumer lag is the clearest leading saturation signal because backlog appears before outright packet loss. If I later introduce a bounded worker executor, queue depth becomes an additional leading signal.
